...
The same goes for working with another for loop, as it helped me to see the way you can apply fastp to multiple files one after the other and just change small parts of the output filename so that each file has the same naming convention within one step.
